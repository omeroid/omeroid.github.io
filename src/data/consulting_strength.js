import React from 'react'
import iconStrength2 from '../assets/images/it/icon-strength-2.png'
import iconStrength4 from '../assets/images/it/icon-strength-4.png'

const consultingStrengthData = () => [
  {
    "image": iconStrength2,
    "title": "ITエンジニアとの一気通貫支援",
    "accent": "accent1",
    "content": [
      "事業開発プロジェクトにおいて、企画設計（Strategy）及びシステム開発（Engineering）を一気通貫でご支援。ビジネス観点とシステム観点両面を加味した設計・提案が可能です", <br />,
    ],
  },
  {
    "image": iconStrength4,
    "title": "広範囲の業界",
    "accent": "accent4",
    "content": [
      "事業戦略・マーケティング戦略・データ/IT戦略の設計を、広範囲の業界に渡って支援してきた豊富な実績と知見があります", <br />,
    ],
  },
]

export default consultingStrengthData
