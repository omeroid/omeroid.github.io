import React from 'react'
import wadaImage from '../assets/images/member/wada.jpg'
import fujitaImage from '../assets/images/member/fujita.jpg'
import kagiwadaImage from '../assets/images/member/kagiwada.jpg'

const consultingMemberData = () => [
  {
    "image": wadaImage,
    "imageAlt": "和田直也",
    "to": null,
    "title": "和田直也 取締役",
    "content": [
      "東京大学経済学部卒業後、経営・ITコンサルティング企業を経て独立、omeroidを共同創業", <br />,
      "・中長期経営戦略からあるべき業務・組織像の設計、デジタル化・DX設計、業務システム設計", <br />,
      "・新規事業立案、MVP、PoC構築支援", <br />,
      "上記のような幅広いテーマでの支援実績多数", <br />,
      "得意業界は製造・流通・ヘルスケア・HR領域、自治体支援", <br />,
    ]
  },
  {
    "image": fujitaImage,
    "imageAlt": "藤田峻平",
    "to": null,
    "title": "藤田峻平 コンサルタント",
    "content": [
      "製造業企業(精密機器・機械)からコンサルタントに転職", <br />,
      "SaaSやWebサービス企業の新事業検討・事業戦略検討支援を担当", <br />,
      "国内短期MBAプログラム(Columbia Venture for all)に参画し、短期ピッチプログラムでは優勝。提案したITサービ スを自らエンジニアとして開発・構築も試行", <br />,
    ]
  },
  {
    "image": kagiwadaImage,
    "imageAlt": "鍵和田祐輔",
    "to": null,
    "title": "鍵和田祐輔 コンサルタント",
    "content": [
      "東京大学医学部卒業後、AIアプリケーション開発・販売企業を経て現職。",
      <br />,
      "大手卸売企業における新規市場参入に向けたマッチングプラットフォームビジネスの検討・構築支援等複数のPJTをメンバーとして推進。”新規事業検討・市場調査分析・プロダクト設計”を専門領域とする。",
      <br />,
    ]
  },
  {
    "to": null,
    "content": [
      "他、専門領域を持つ多数のコンサルタント所属",
    ]
  }
]

export default consultingMemberData
